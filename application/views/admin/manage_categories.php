	   ...
	   <form name="frmCatOutlet" id="frmCatOutlet" action="<?php echo ADMIN_BASE_URL?>category/manage" method="post">
        ...
        <div class="row <?php if($s_no%2 == 0){echo "even";}?>" id="row_<?php echo $row['id']?>" >
            ...
            <label class="action">
            	...
            	<a href="<?php echo ADMIN_BASE_URL.'category/add/'.$row['id']?>" class="icon_edit" title="Edit">&nbsp;</a>
...
$(document).ready(function(){
	$(".icon_delete").click(function(){
		...
		$.ajax({
			  ...
			  data: 'table=category&id='+id,
			  success: 
			 	function(msg){
				  if(msg == 1)
				  {
					 	alert('Category is deleted successflly.'); 
	...
	$(".icon_status").live("click" ,function(){
		...
		$.ajax({
			  ...
			  data: 'table=category&id='+id+'&status='+status,
			  success: function(msg){
				  if(status == 1)
				  {
					...
					alert("Category is deactivated successfully");
				  }
				 else
				 {
					...
					alert("Category is activated successfully");
				 }
			  }
		  });
	});
