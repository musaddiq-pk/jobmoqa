...
class ad extends CI_Controller 
{
	...
	{	
		...
		$ad_data['where'] = array('ad_date > '=>$prev_date,'status'=>1); //only one week older jobs
		$ad_data['segment'] = array('limit'=>10,'offset'=>$segment);
		$ad_data['order'] = array('order_by'=>'ad.ad_date'); //Order by two different fileds cdate and visits...
		...
		$data['title'] = 'All Newspaper Latest ('.date('Y').') Jobs (2018) In Pakistan | '.SITE_NAME;
		$data['meta_desc'] = 'Get the latest ('.date('Y').') Job alerts in Pakistan published in major newspapers like Jang, Express, Nawa-I-Waqt, The News, Dawn, The Nation, Daily Aaj, Ausaf, Kawish, Khabrain and Observer with JobMoqa';//$page['seo_meta_desc'];
		...
		$data['is_home'] = true;
		//echo '<pre>';print_r($data['menu']);echo '</pre>';exit;
	...
	{	
		...
		$data['title'] = 'Latest ('.date('Y').') '.$arr_cat['name'].' Jobs In Pakistan |  '.SITE_NAME;//$arr_cat['seo_title']. ' | '.SITE_NAME;
		$data['meta_desc'] = 'Get Latest ('.date('Y').') '.$arr_cat['name'].' jobs alerts in Pakistan from Jang, Express, Nawa-I-Waqt, The News, Dawn, The Nation, Daily Aaj, Ausaf, Kawish, Khabrain and Observer with JobMoqa';//$arr_cat['seo_meta_desc'];
	...
	{	
		...
		$this->pagination->initialize($config); 
		$data['paging'] =  $this->pagination->create_links();
		//echo '<pre>'.$data['paging'] ;echo '</pre>';exit;
		...
		$data['title'] = $arr_paper['name'].' Latest ('.date('Y').') | '.SITE_NAME;//$arr_paper['seo_title']. ' | '.SITE_NAME;
		$data['meta_desc'] = 'Get all latest ('.date('Y').') '.$arr_paper['name'].' Jobs alerts in Punjab, Sindh, Khyber Pakhtoonhwa (KPK), Balochistan, Gilgit Biltistan, Azad Kashmir (AJK) and all over Pakistan with JobMoqa.';//$arr_paper['seo_meta_desc'];
	...
	{	
		...
		$key_word = '';
		$start_date = '';//$this->general_lib->get_curr_date();
		$end_date = '';//$this->general_lib->get_prev_date('-60 days');
		$paper_id = 0;
		...
		$search_data = array('key_word'=>$key_word,'start_date'=>$start_date,'end_date'=>$end_date,'paper_id'=>$paper_id);
		...
		$ad_count['where'] = $ad_data['where'] = array();
		
		if($key_word != '')
		{
			$str = "(name like '%$key_word%' OR `desc` like '%$key_word%' OR seo_meta_desc like '%$key_word%')";
			$ad_count['like'] = $ad_data['like'] = $str;
			/*$ad_count['where']['names like '] = $ad_data['where']['name like '] = '%'.$key_word.'%';
			$ad_count['where']['desc like '] = $ad_data['where']['desc like '] = '%'.$key_word.'%';
			$ad_count['where']['seo_meta_desc like '] = $ad_data['where']['seo_meta_desc like '] = '%'.$key_word.'%';*/
			
		}
		if($start_date != '')
			$ad_count['where']['ad_date >='] = $ad_data['where']['ad_date >='] = $this->general_lib->set_db_date($start_date);
		if($end_date != '')
			$ad_count['where']['ad_date <='] = $ad_data['where']['ad_date <='] = $this->general_lib->set_db_date($end_date);	
		if($paper_id != 0)
			$ad_count['where']['ad_newspaper.newspaper_id'] = $ad_data['where']['ad_newspaper.newspaper_id'] = $paper_id;
	...
	{	
		...
		$ad_papers = '';
		foreach($ad_info['papers'] as $p)
			$ad_papers .= $p['name'].',';
		$ad_papers = trim($ad_papers,',');
		
		$data['menu'] = $this->general_model->get_menu();
		$data['page_heading'] = $ad_info['name'];
		$data['title'] = $ad_info['seo_title'].' | '.SITE_NAME;
		$data['meta_desc'] = $ad_info['seo_title'].' published in '.$ad_papers.' by '.SITE_NAME;
		...
		$fb_info = array();
		$fb_info['url'] = $url;
		$fb_info['type'] = SITE_NAME;
		$fb_info['title'] = $data['title'];
		$fb_info['description'] = $data['desc'];
		$fb_info['image'] = AD_LARGE_IMG_PATH.$ad_info['image'];
	...
	function all_news_papers()
	{
		...
		$data['title'] = 'Lates Jobs ('.date('Y').') in Pakistan From All NewsPaper | '.SITE_NAME;
		$data['meta_desc'] = 'Get latest ('.date('Y').') job alerts in Pakistan from Jang, Express, Nawa-i-Waqt, The News, Dawn &amp; The Nation Newspapers with jobmoqa.';
		
		$data['title'] = 'Latest Jobs ('.date('Y').') in Pakistan From All NewsPaper | '.SITE_NAME;
		$data['meta_desc'] = 'Get latest jobs ('.date('Y').') alerts in Pakistan from Jang, Express, Nawa-i-Waqt, The News, Dawn &amp; The Nation Newspapers with jobmoqa.';
	...
	function categories()
	{
		...
		foreach($arr_cats as $cat)
		{
			...
			$ad_data['limit'] = array('limit'=>3,'offset'=>0);
		...
		$data['title'] = 'Latest Jobs ('.date('Y').') Of All Departments in Pakistan From All NewsPaper | '.SITE_NAME;
		$data['meta_desc'] = 'Get the latest ('.date('Y').') jobs alerts of all departments in Pakistan from major newspapers with jobmoqa.';
	...
	function regions()
	{
		...
		$data['title'] =  'Get Latest ('.date('Y').') Jobs All Over Pakistan | '.SITE_NAME;
		$data['meta_desc'] = 'Find the latest ('.date('Y').') government jobs all over Pakistan published in major newspapers with JobMoqa.';
	...
	function region($url='')
	{
		...
		$data['title'] =  'Get Latest jobs ('.date('Y').') In '.$arr_region['name'].' | '.SITE_NAME;
		$data['meta_desc'] = 'Find the latest ('.date('Y').') jobs in '.$arr_region['name'].' of all departments published in major newspapers of Pakistan with JobMoqa.';
	...
	{
		$session_data = array();
		$session_data['start_date'] = $_POST['variables'][0] ;
